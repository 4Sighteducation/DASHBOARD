-- Fix the distribution arrays to use correct score ranges (1-6 for elements, 1-10 for overall)
        ...
        SELECT 
            ...
            CASE 
                WHEN sd.element = 'overall' THEN
                    -- For overall: 10 elements for scores 1-10
                    ARRAY(
                ...
                ELSE
                    -- For other elements: 6 elements for scores 1-6
                    ARRAY(
                        SELECT COALESCE(dc.score_count, 0)
                        FROM generate_series(1, 6) AS gs(val)
                        LEFT JOIN distribution_calc dc 
                            ON dc.establishment_id = sd.establishment_id 
                            AND dc.cycle = sd.cycle 
                            AND dc.academic_year = sd.academic_year 
