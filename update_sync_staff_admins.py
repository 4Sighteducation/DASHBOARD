#!/usr/bin/env python3
"""
Update sync script to properly populate establishment_id for staff_admins
This is CRITICAL for the dashboard to work correctly
"""
    ...
    for record in knack_records:
        try:
            ...
            if 'field_201' in record and record['field_201']:
                # This field contains the connected establishment record
                if isinstance(record['field_201'], list) and len(record['field_201']) > 0:
                    establishment_knack_id = record['field_201'][0]['id']
                elif isinstance(record['field_201'], dict):
                    establishment_knack_id = record['field_201']['id']
            ...
            if existing.data:
                ...
                supabase.table('staff_admins').update(staff_admin_data).eq('knack_id', knack_id).execute()
                logger.info(f"Updated staff admin: {email} -> Establishment: {establishment_id}")
            else:
                # Insert new record
                staff_admin_data['created_at'] = datetime.now().isoformat()
                supabase.table('staff_admins').insert(staff_admin_data).execute()
